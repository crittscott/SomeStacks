...
Which items count as ingots is a server setting (see [Server administration](server-admin.md)) — by default, everything under the `forge:ingots` tags plus the mod's own `somestacks:ingots` tag, which a data pack can extend.
