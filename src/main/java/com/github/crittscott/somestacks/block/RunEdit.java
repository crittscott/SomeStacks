 ...
 * extraction can take blocks off it. Those world edits notify neighbours, and a neighbour woken in
 ...
 * nothing, which is an answer every caller already handles, and the run is left in the state the
 * outer call is building. Only mutations are held off. A simulation reads and computes without
 * touching the world, so it stays available to a caller that asks in the middle of one.
 *
