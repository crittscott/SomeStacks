...
There is no continuously ticking block entity. Work happens in response to interaction, capability access, configuration events, or resource reloads. What is deferred is publication: an edit schedules a block tick on the bottom of the run it touched, so a burst of edits collapses into one pass. For Storage that pass is the pile settle. For Singles and Bar it is the content sync, light recomputation and comparator publication the edit leaves owing.
...
All three block entities expose Forge's item-handler capability on every side, and in each case every block of a vertical run exposes the whole run as one inventory, so a hopper under the base and an interface halfway up address the same contents.

Every operation on those handlers is positional: it addresses the slot it was given and answers for that slot alone. So a caller that walks the advertised range and sums what each slot accepts gets the run's real capacity, and a simulation at one slot promises exactly what a commit at that slot delivers. What differs between the three is what one slot is. A Storage slot is a place in a bag and holds a stack, so it takes what a slot takes and the pile's settle packs it down to the base on the next tick. A Singles cell and a Bar position are places in a structure and hold one item each, so one call places one item, and only where that position is empty and standing on something — an unsupported position is refused rather than redirected to one that would take the item, because a slot index that means one place cannot also mean whichever place happens to work.

A caller walking the range in order still fills a structure from the bottom, because each placement stands before the next position is offered: a filled layer supports the layer above it by the time the walk arrives there. What such a walk cannot do is promise that in advance. A simulation reads occupancy as it stands, so it counts only the positions supported at that instant and reports less than a walk will actually place. That is the safe direction — a caller is never promised more than it gets — and it is the cost of an answer that belongs to one position rather than to the whole run.
...
A run advertises the positions it holds plus one block's worth of headroom, while the configured height allows another block. Neither extreme serves: advertising the full permitted height leaves most of the range permanently empty, and a caller that walks it — a storage network's external storage polls its inventory every tick — spends the great majority of every scan on positions that cannot exist; advertising only what is there means a caller offers items to the positions the range names and no others, so a full run would never be offered the insertion that grows it. One block is what one growth adds, so the advertised range is reachable capacity and nothing more.

The headroom positions are where a run grows. An insertion aimed at one of them adds the block first and then stores into it, and a simulation aimed at one answers whether that block could be placed at all — the configured height, the type's enable flag, build height, replaceability, the world border and vanilla spawn protection, which is one predicate per type shared by the simulation and the commit so that neither can promise what the other refuses. Spawn protection exempts operators, so the answer depends on who is growing the run: a player's own deposit is weighed against that player, and a capability insertion against the level's fake player, which is never exempt. Forge's block-place event is the one thing a simulation cannot consult, because firing it is the only way to ask; that is the documented residual. A structure's headroom position must also stand on the run's current top layer, since a block added above an empty top layer would have nothing to support its first item. A run that automation is filling grows as far as it likes, one block at a time, over as many insertions as it takes.
...
| Gesture | Result |
| --- | --- |
| Hold `V` and right-click air | Cycle Storage, Singles, Bar, and Toggle Permanent modes, showing the selected mode in the action bar. The code does not require Shift. Synced-disabled block-type modes are skipped. |
...
The bottom layer of a stacked block rests on the seam, the top layer of the Singles Stack below, recorded in visual columns so it means the same thing to a block above of any rotation. A block standing on the world rather than on another Singles Stack has its bottom layer grounded outright. The seam governs deposits as well: a bottom-layer item may only be placed where the block below supports it, on every deposit into a stacked block rather than only the first one that creates it. Grounding is checked in one place, the block entity's deposit, which is the only caller holding the seam beneath it; the packet handlers target a cell and leave support to it. The one exception is a block that does not exist yet, whose first item is weighed against the seam it would stand on before the placement happens.
...
A vertical run of Singles Stacks is one inventory to the item-handler capability, addressed from any block in it, with positions running from the bottom block's first cell upward. A slot index is a place in a structure rather than a place in a bag, so both operations address the cell they are given:

- **Insertion** places one item in the cell named, and only where that cell is empty and supported; a cell in the headroom above the column grows it first. An unsupported cell is refused. A caller walking the cells in order therefore fills the column layer by layer from the bottom, and fills a gap lower down before anything higher, because each item stands before the next cell is offered. Growth carries no player, so it is attributed to the level's fake player and runs the same protection path as any placement. A block grown this way is unrotated, as a player's own placement is; the seam is read in visual columns, so a grown block's frame need not match the one beneath it.
...
A vertical run of Bar Stacks behaves as one stack. After one bar is extracted, the block scans the remaining bars from the bottom layer up. Any bar without an overlapping support footprint beneath it is removed and dropped into the world. One pass suffices per block: a layer is only ever supported by the layer beneath it, which the pass has already settled. A collapse collects every drop until the structure is settled, then packs exact item identities up to their maximum stack size at each block they fell from before spawning item entities.
...
The seam also governs deposits: a bottom-layer bar may only be placed where the Bar Stack below supports it. This holds for every deposit into a stacked block, not just the first one that creates it. Grounding is checked in one place, the block entity's deposit, which is the only caller holding the seam beneath it; the packet handlers target a cell and leave support to it. The one exception is a block that does not exist yet, whose first bar is weighed against the seam it would stand on before the placement happens.
...
A vertical run of Bar Stacks is one inventory to the item-handler capability, addressed from any block in it, with positions running from the bottom block's first cell upward. A slot index is a place in a structure rather than a place in a bag, and both automated operations address the position they are given:

- **Insertion** places one bar at the position named, and only where that position is empty and its footprint overlaps an occupied bar beneath it; a position in the headroom above the column grows it first. An unsupported position is refused. A caller walking the positions in order therefore fills the column layer by layer from the bottom, because each bar stands before the next position is offered. Growth carries no player, so it is attributed to the level's fake player and runs the same protection path as any placement.
...
Both operations preserve density: a column whose bars occupy a contiguous run of positions still does afterwards, so automation neither leaves gaps among filled positions nor drops anything on the floor. A column a player built sparsely is not made worse, and a walk of the positions fills its lower gaps first.
...
A content change owes three things beyond the contents themselves: a block entity update to clients, a recomputed light level for that block, and the run's comparator output. None of them is due at the instant of the edit, and all three are expensive enough that paying them per edit dominates the edit.

So an edit records that the block owes a publication and schedules a block tick on the bottom of its run. The tick pays what is outstanding: contents and light for each block that has an edit pending, then the comparator output once for the whole run. Scheduling on the bottom is what makes a burst coalesce — every edit anywhere in a run marks the same position, and the scheduling is guarded on there being no tick already pending there.

This matters most where a slot holds one item. A Singles cell and a Bar position each take exactly one, so a caller moving a stack through the capability makes sixty-four calls, and a single Singles removal walks the column drawing an item down out of every block above it. Immediate publication charged each of those the full price; deferral charges one pass per block per tick.

Within one operation, gravity and settling also suppress per-slot sync while they run, so a cascade marks each block it touches once rather than once per slot it moves. Storage additionally tracks which blocks a batch really changed and leaves the others alone.

Two things stay immediate, because a later operation in the same tick depends on them: the stored contents, and the removal of blocks a pass empties. What defers is only the telling.

A run that loses a block from the middle becomes two runs, and the tick scheduled on the old bottom will never reach the upper one, so a removal schedules both sides again.
...
3. The bundled corpus: all JSON under `assets/*/item_render_overrides/`, loaded by client resource reload. Every namespace is scanned, so a resource pack contributes under its own. A file is named for the namespace of the items it corrects and corrects no others, and that name is load-bearing rather than a convention: the corpus is cross-mod compatibility data, the great majority of which any one client cannot use, so a file naming a namespace the client does not have is skipped unread rather than parsed and retained. A file whose name is not a namespace reaches nothing. A namespace counts as present when the item registry holds an item under it or the mod list names it — the registry is the exact answer and covers a mod registering items under some other namespace, while the mod list is fixed before any mod bus event fires and so cannot be read too early. Neither can change within a session. The shipped corpus is 231 files and some 46,000 entries, of which a vanilla client reads one file.
...
The client mode cycler skips every synced-disabled block-type mode, so a mode the server would refuse is never offered. Toggle Permanent belongs to no block type and is always in the cycle. With all three types disabled the cycle returns to where it started rather than spinning.
...
The automated suite has two deliberately separate layers, 51 JUnit tests and 69 Forge GameTests.
...
- Storage pile behavior: `StoragePile` owns pile resolution, bottom-up filling for a player's deposit, the capability's positional insertion, growth, the flat slot range, settling, and trimming. A Storage Stack that needs to reach past its own 27 slots resolves a pile rather than walking the column itself.
- Singles column behavior: `SinglesColumn` owns column resolution, the capability's positional insertion and the support test behind it, growth, comparator output and its publication, the deferred publication pass, and the flat slot range. Extraction routes back through the block entity's own removal, so the column has one gravity implementation rather than a player one and an automated one.
- Bar column behavior: `BarColumn` owns column resolution, the capability's positional insertion and the support test behind it, growth, comparator output and its publication, the deferred publication pass, the flat slot range, and the backfilling extraction. `BarCubeIdx` reasons over occupancy snapshots, so a placement can be weighed before it happens and a block can be settled in place.
- Capability semantics: an operation answers for the slot it was given and nothing else. A structure refuses a position it cannot support rather than choosing another, and a simulation reads the world as it stands rather than planning ahead. Both are what keep a walk of the advertised range summing to the truth.
...
- Ordinary item compatibility: prefer an override entry — authored in game with `ss item` and `ss write`, or edited into the bundled `item_render_overrides/` corpus — before changing the global rendering paths. A corpus entry goes in the file named for its own namespace, since the loader reads a file only when that namespace is present; a JUnit case holds the corpus to that.
