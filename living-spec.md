...
- Naming a single namespace fails when that namespace is disabled or has nothing to show, because the command names one thing and it did not happen. `all` and `list` instead skip such a namespace and report it, so one bad entry does not withhold the rest of the wall. The two reasons are reported apart: a disabled namespace is doing what the server was told, while an unloaded one — or one holding none of that kind's items — is usually a typo, or an entry meant for the other kind, since one list serves both.
...
- `ss gen add|remove|list` edits `gen_mods`. It is an operator command rather than an allow-listed one because it edits server config, while running the wall it describes stays with the allow list. Adding completes over every namespace that has items, not over one kind's, since the one list serves both `ss test list` and `ss testingot list`. This list alone is reported in stored order rather than sorted, because that order is the order of the wall's columns.
