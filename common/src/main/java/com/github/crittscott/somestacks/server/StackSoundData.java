    ...
    private static JsonObject merge(Map<ResourceLocation, JsonElement> files) {
        ...
        ordered.sort(Comparator
                .comparing((Map.Entry<ResourceLocation, JsonElement> e) ->
                        !e.getKey().getNamespace().equals(SomeStacksCommon.MODID))
        ...
        for (Map.Entry<ResourceLocation, JsonElement> entry : ordered) {
            ...
            if (!entry.getValue().isJsonObject()) {
                SomeStacksCommon.LOGGER.warn("Ignoring sound definition {}: root is not a JSON object", id);
                continue;
            }
            ...
            for (String blockType : root.keySet()) {
                ...
                if (actions == null) {
                    SomeStacksCommon.LOGGER.warn("Ignoring unknown block type '{}' in sound definition {}", blockType, id);
                    continue;
                }
                if (!root.get(blockType).isJsonObject()) {
                    SomeStacksCommon.LOGGER.warn("Ignoring '{}' in sound definition {}: not a JSON object", blockType, id);
                    continue;
                }
                ...
                for (String action : blockObj.keySet()) {
                    if (!actions.contains(action)) {
                        SomeStacksCommon.LOGGER.warn("Ignoring unknown action '{}.{}' in sound definition {}",
                                blockType, action, id);
                        continue;
                    }
    ...
    private static SoundEvent parseSound(JsonObject root, String blockType, String action, SoundEvent fallback) {
        try {
            ...
            if (blockObj == null || !blockObj.has(action)) {
                SomeStacksCommon.LOGGER.warn("No {} sound defined for {}, using fallback", action, blockType);
                return fallback;
            }

            String soundName = blockObj.get(action).getAsString();
            ResourceLocation loc = ResourceLocation.tryParse(soundName);
            SoundEvent sound = loc == null ? null : BuiltInRegistries.SOUND_EVENT.getOptional(loc).orElse(null);
            if (sound == null) {
                SomeStacksCommon.LOGGER.warn("Unknown sound '{}', using fallback", soundName);
                return fallback;
            }
            return sound;
        } catch (Exception e) {
            SomeStacksCommon.LOGGER.warn("Failed to parse sound for {}.{}: {}", blockType, action, e.getMessage());
            return fallback;
        }
    }
}
